...
namespace Archer
{
  Koios_C::Koios_C(TripENC &tENC, ELMO_CANt4 &elmo):
  tENC_(tENC),
  elmo_(elmo)
  {
    pinMode(_sIN,INPUT);
  ...
  int32_t Koios_C::initComm(int MC){
    setLEDs("1000");
    delay(10);
  ...
  void Koios_C::initKoios1(int MC){
    flashR(1);
    delay(6000);
    rt = initComm(MC);
    delay(1000);
  ...
  void Koios_C::initKoios2(){
    delay(250);
    STO(1);
    waitSwitch();
    delay(1000);
    rt = elmo_.motorOn(IDX_K1);
    rt = elmo_.motorOn(IDX_K2);
    rt = elmo_.motorOn(IDX_K3); // Combine these, with a single return code
    flashA1(1);
    delay(7500);
    resetStates();
    setLEDs("0001");
  ...
  void Koios_C::setLEDs(String val){
    if(val[0]=='1'){
  ...
  void Koios_C::setLogo(char Color){
    if(Color == 'R'){
      digitalWrite(_relayA,LOW);
      digitalWrite(_relayG,LOW);
  ...
  void Koios_C::setSigB(int val){
    digitalWrite(_sigB,val);
  }

  int Koios_C::checkSwitch(){
    int s1;
    s1 = digitalRead(_sIN);
    return s1;
  }

  void Koios_C::waitSwitch(){
    int a;
    int s;
    a = 0;
    while(a==0){
  ...
  void Koios_C::STO(int val){
    digitalWrite(_relayK,val);
  }

  int32_t Koios_C::motorsOff(int val){
  ...
  void Koios_C::resetState(int encID){
    uint32_t tmp  = micros();
    long     tcnt = tENC_.readEncoder(encID);
    if(encID==1){
      _prevT1 = tmp;
      _prevC1 = -tcnt;  // This has been inverted
    }
    if(encID==2){
      _prevT2 = tmp;
      _prevC2 = -tcnt;  // This has been inverted
    }
    if(encID==3){
      _prevT3 = tmp;
      _prevC3 = tcnt;
  ...
  void Koios_C::updateState(int encID, float &x, float &v){
    uint32_t tmp  = micros();
    long     tcnt = tENC_.readEncoder(encID);
    float    pX,dt;
    if(encID==1){
  ...
  void Koios_C::updateStates(float &x1,float &v1,float &x2,float &v2,float &x3,float &v3){
    float    pX,dt;
    uint32_t tmp  = micros();
    long     tcnt = tENC_.readEncoder(1);
    _nextT1 = tmp;
  ...
  int Koios_C::checkFrame(float q0,float q1,float q2,float q3,float dY,float dP,float dR){
    int rt;
    _q0 = q0;
    _q1 = q1;
    _q2 = q2;
  ...
  int Koios_C::updateAtt(float &Yaw,float &Pitch,float &Roll){
  ...
  void Koios_C::track(float x1d,float v1d,float x2d,float v2d,float x3d,float v3d){
    float x,v,u;
    updateState(1,x,v);
    u = -_KpK*(x-x1d) - _KdK*(v-v1d);
    rt = elmo_.sendTC(u,1);
  }

  void Koios_C::comp(float d){
    // do nothing yet
  }

  void Koios_C::delayLoop(uint32_t T0, uint32_t dTdes){
    uint32_t T1 = micros();
    if ((T1 - T0) < dTdes)
