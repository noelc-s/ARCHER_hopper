...
public:
    ...
    //  \in     q, v, a  - state to evaluate the dynamics at
    //  \out    x_dot  - the dynamics (dq, ddq)
    vector_t f(const vector_t& q, const vector_t& v, const vector_t& a, const domain& d);

    //  \brief  compute the linearizations of f
    //  \in     q, v, a  - state to compute the jacobians at
    //  \out    A, B, C  - df/dx, df/du, and the residual
    ...
    //  \in     Ac, Bc, Cc - continuous time dynamics of the system
    //  \out    Ad, Bd, Cd - discrete time dynamics of the system
    ...
    //  \in     q, v, J - state to compute the impact map at, and Jacobian of constraint
    //  \out    x_plus   - the post impact state (q+, dq+)
    ...
    //  \in     q, v, J - state to compute the impact map at, and Jacobian of constraint
    //  \out    A, B, C   -  ddelta_f/dx, ddelta_f/du, and the residual
