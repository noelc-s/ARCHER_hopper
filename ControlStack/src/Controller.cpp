...
int main()
{
  ...
  // std::unique_ptr<Command> command = std::make_unique<SingleIntCommand>(10);
  std::unique_ptr<Command> command = std::make_unique<DoubleIntCommand>(10);
  std::cout << command->getStateDim() << std::endl;
  RLTrajPolicy policy = RLTrajPolicy("../../models/hopper_traj_double_int_cur2_8qcxerd5.onnx", gainYamlPath, command->getHorizon(), command->getStateDim());
  ...
  // Instantiate command
  // std::unique_ptr<Command> command = std::make_unique<V3Command>();
  

  std::thread getUserInput(&UserInput::getJoystickInput, &readUserInput, std::ref(offsets), std::ref(command), std::ref(reset), std::ref(cv), std::ref(m));
